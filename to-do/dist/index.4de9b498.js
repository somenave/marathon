...
const DONE = 'Done';
let list = [];
_viewJs.UI_ELEMENTS.todoForms.forEach((form)=>{
...
function createTaskLi(priorityContainer) {
    ...
    _viewJs.UI_ELEMENTS.currentTaskText = currentTaskContent.querySelector('.todo-task__text');
    let taskName;
    list.forEach((task)=>{
        if (task.id == currentTaskContent.getAttribute('data-id')) taskName = task.name;
    });
    _viewJs.UI_ELEMENTS.currentTaskText.textContent = taskName;
    console.log(_dateFns.format(new Date(), 'PP'));
}
function addTask(priorityContainer) {
    priorityContainer.querySelector('.add-task-icon').addEventListener('click', ()=>{
        _viewJs.UI_ELEMENTS.addTaskInput = priorityContainer.querySelector('.todo-add .add-task-input');
        try {
            if (_viewJs.UI_ELEMENTS.addTaskInput.value.trim() !== '') {
                list.push({
                    id: list.length + 1,
                    name: _viewJs.UI_ELEMENTS.addTaskInput.value,
                    status: TO_DO,
                    priority: priorityContainer.getAttribute('data-priority')
                });
                createTaskLi(priorityContainer);
            } else throw new Error("You cannot add an empty task");
        } catch (e) {
            alert(e.message);
        }
        removeTask();
        isDone();
        _viewJs.UI_ELEMENTS.todoForms.forEach((form)=>form.reset()
        );
    });
}
addTask(_viewJs.UI_ELEMENTS.highTasksContainer);
addTask(_viewJs.UI_ELEMENTS.lowTasksContainer);
function isDone() {
    document.querySelectorAll('.done-input').forEach((item)=>{
        item.addEventListener('change', ()=>{
            let status = TO_DO;
            list.forEach((task)=>{
                if (task.id == item.parentElement.parentElement.getAttribute('data-id')) task.status = status;
            });
            if (item.checked) {
                status = DONE;
                item.closest('.todo-task').classList.add('done');
            } else item.closest('.todo-task').classList.remove('done');
        });
    });
}
isDone();
function removeTask() {
    document.querySelectorAll('.close').forEach((item)=>{
        ...
        item.addEventListener('click', ()=>{
            if (list.length >= 1) {
                list.forEach((task)=>{
                    if (task.id == removedTask.getAttribute('data-id')) list.splice(task.id - 1, 1);
                });
                removedTask.remove();
            } else list = [];
        });
...
},{"date-fns":"9yHCA","./view.js":"2GA9o"}],"9yHCA":[function(require,module,exports) {
...
const UI_ELEMENTS = {
    todoForms: document.querySelectorAll('.todo-form'),
    addTaskInputs: document.querySelectorAll('.add-task-input'),
    addTaskInput: document.querySelector('.add-task-input'),
    highTasksContainer: document.querySelector('.high-list'),
    lowTasksContainer: document.querySelector('.low-list'),
    currentTaskText: document.querySelector('.todo-task__text')
};

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["dRf9z","4pp4s"], "4pp4s", "parcelRequired9c2")
